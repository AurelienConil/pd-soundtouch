...
SoundTouchExt::SoundTouchExt(const PdArgs &a)
    : SoundExternal(a), pitch_value_(DEF_PITCH), drywet_(0, 1)
{
    createSignalOutlet();
    createInlet();
    ...
    createCbBoolProperty(
        "@antialias",
        [this]() -> bool
        { return stouch_.getSetting(SETTING_USE_AA_FILTER); },
        [this](bool b) -> bool
        { return stouch_.setSetting(SETTING_USE_AA_FILTER, b); });

    // antialias length
    auto alen = createCbIntProperty(
        "@aalength",
        [this]() -> int
        { return stouch_.getSetting(SETTING_AA_FILTER_LENGTH); },
        [this](int v) -> bool
        { return stouch_.setSetting(SETTING_AA_FILTER_LENGTH, v); });
    alen->setDefault(8);
